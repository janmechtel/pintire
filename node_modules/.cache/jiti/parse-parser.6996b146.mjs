...
class Parser {
  ...
  *pop(error) {
    ...
    } else
    {
      ...
      if (token.type === 'flow-collection' && top.type === 'document') {
      ...
      switch (top.type) {
        ...
        case 'flow-collection':{
            const it = top.items[top.items.length - 1];
            if (!it || it.value)
            top.items.push({ start: [], key: token, sep: [] });else
            if (it.sep)
            it.value = token;else
    ...
    switch (this.type) {
      case 'newline':
        ...
        if (it.value) {
          ...
          end?.push(this.sourceToken);else
      ...
      switch (this.type) {
        ...
        case 'map-value-ind':
          if (it.explicitKey) {
            ...
            if (includesToken(it.sep, 'map-value-ind')) {
              this.stack.push({
                type: 'block-map',
                offset: this.offset,
                indent: this.indent,
  ...
  *blockSequence(seq) {
    ...
    switch (this.type) {
      ...
      case 'comment':
        if (it.value)
        seq.items.push({ start: [this.sourceToken] });else
        {
  ...
  *flowCollection(fc) {
    ...
    if (fc.end.length === 0) {
      switch (this.type) {
        ...
        case 'map-value-ind':
          if (!it || it.value)
          fc.items.push({ start: [], key: null, sep: [this.sourceToken] });else
          if (it.sep)
...
exports.Parser = Parser; /* v9-127c973f8b557106 */
